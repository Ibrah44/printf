...
````
This is the first group project that we have at ALX School, which consists of replicating the **[printf (3)](http://man7.org/linux/man-pages/man3/printf.3.html)** function of language c, calling it this way **_printf.**
...
````
## [Flowchart.](https://photos.app.goo.gl/WxbnvUKcZGeRrqHY7)
## How to use.
### Complilation
All of the ``.c`` files along with a main.c file are to be compiled with ``gcc`` on Ubuntu 20.04 LTS with the flags ``-Wall Werror`` ``-Westra`` ``-pedantic.`` and ``-std=gnu89.`` 
...
```
```{bash}
linux>$  gcc -Wall -Werror -Wextra -pedantic std=gnu89 *.c -o print_program
linux>$  ./print_program
Hello, ALX is 89.
linux>$
```
